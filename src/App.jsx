...
const PLAN = {
  ...
  thursday:{
    ...
    focus:"Deadlift & Core", subtitle:"Half Day · 45 min",
    exercises:[
      {name:"Treadmill Warm-Up", machine:"Treadmill", type:"cardio", isCardio:true,
       sets:[{duration:8,speed:6.5,incline:3}], note:"Incline 3%, activates glutes"},
      {name:"Barbell Deadlift", machine:"Barbell Rack", type:"back",
       sets:[{reps:8,weight:60},{reps:8,weight:60},{reps:6,weight:70},{reps:6,weight:70}],
       note:"Feet hip-width. Bar over mid-foot. Flat back, chest up. Drive through floor. Bar stays close to body the whole way up."},
      {name:"Lat Pulldown Wide", machine:"Lat Pulldown Machine", type:"back",
       sets:[{reps:12,weight:45},{reps:12,weight:45},{reps:10,weight:50},{reps:10,weight:50}],
       note:"Pull to upper chest. Lean back slightly. Full stretch at top."},
      ...
      {name:"Ab Crunch Machine", machine:"Ab Crunch Machine", type:"core",
       sets:[{reps:20,weight:20},{reps:20,weight:20},{reps:15,weight:25},{reps:15,weight:25}],
       note:"Slow squeeze. Hold 1 sec at bottom. Full range of motion."},
    ],
  },
  friday:{
    ...
    exercises:[
      ...
      {name:"Cable Face Pull", machine:"Cable Machine (rope)", type:"back",
       ...
       note:"Elbows high. Pull to forehead. Great for posture and rear delts."},
      {name:"Barbell Bent Over Row", machine:"Barbell Rack", type:"back",
       sets:[{reps:10,weight:35},{reps:10,weight:35},{reps:8,weight:40}],
       note:"Hinge at hips 45°. Pull bar to belly button. Squeeze shoulder blades. Keep back flat throughout."},
  ...
  saturday:{
    ...
    exercises:[
      ...
      {name:"Assisted Pull-Up Machine", machine:"Pull-Up Assist Machine", type:"back",
       sets:[{reps:8,weight:30},{reps:8,weight:30},{reps:6,weight:25}],
       note:"Weight shown is assistance — less = harder. Full hang at bottom, chin over bar."},
      {name:"Bicep Curl Cable", machine:"Cable Machine (EZ bar)", type:"arms",
      ...
      {name:"Ab Crunch Machine", machine:"Ab Crunch Machine", type:"core",
       sets:[{reps:20,weight:20},{reps:20,weight:20},{reps:15,weight:25},{reps:15,weight:25}],
       note:"Slow squeeze. Hold 1 sec at bottom. Add weight when it gets easy."},
  ...
  sunday:{
    ...
    exercises:[
      ...
      {name:"Lat Pulldown Light", machine:"Lat Pulldown Machine", type:"back",
       sets:[{reps:15,weight:30},{reps:15,weight:30}],
       note:"Light weight. Stretch and mobility focus."},
      {name:"Ab Crunch Machine Light", machine:"Ab Crunch Machine", type:"core",
       ...
       note:"Light weight. Core activation and recovery only."},
    ],
  },
};
    ...
    <div style={{background:"#F5F5F5",minHeight:"100vh",maxWidth:430,margin:"0 auto",fontFamily:"-apple-system,'SF Pro Display',sans-serif",position:"relative",paddingBottom:70}}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-pill{border:none;border-radius:100px;cursor:pointer;font-size:14px;font-weight:700;padding:14px 28px;letter-spacing:.3px;transition:all .15s;-webkit-tap-highlight-color:transparent}
        .btn-sm{border:none;border-radius:100px;cursor:pointer;font-size:12px;font-weight:700;padding:8px 16px;transition:all .15s;-webkit-tap-highlight-color:transparent}
        .btn-icon{border:none;border-radius:50%;cursor:pointer;width:44px;height:44px;display:flex;align-items:center;justify-content:center;font-size:18px;transition:all .15s;-webkit-tap-highlight-color:transparent}
        input{background:#F5F5F5;border:none;border-radius:12px;color:#1a1a1a;font-family:inherit;font-size:14px;padding:12px 14px;width:100%;outline:none;transition:box-shadow .2s}
        input:focus{box-shadow:0 0 0 2px #FF5722}
        .set-row{display:grid;align-items:center;padding:10px 16px;border-bottom:1px solid #F5F5F5;gap:8px}
        .num-in{width:56px;text-align:center;padding:8px 4px;font-size:15px;font-weight:700;border-radius:10px;background:#F5F5F5;border:none;font-family:inherit;outline:none;-webkit-tap-highlight-color:transparent}
        .num-in:focus{box-shadow:0 0 0 2px #FF5722}
        .check-btn{width:32px;height:32px;border-radius:50%;border:2px solid #E0E0E0;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .2s;flex-shrink:0;-webkit-tap-highlight-color:transparent;font-size:14px}
        .check-btn.done{background:#4CAF50;border-color:#4CAF50;color:#fff}
        .water-btn{flex:1;border:2px solid #E3F2FD;background:#E3F2FD;border-radius:12px;padding:12px 4px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;transition:all .15s;-webkit-tap-highlight-color:transparent}
        .water-btn:active{transform:scale(.96)}
        .tab-btn{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:8px 0;background:transparent;border:none;cursor:pointer;-webkit-tap-highlight-color:transparent;transition:all .15s}
        .toast{position:fixed;top:20px;left:50%;transform:translateX(-50%);padding:10px 22px;border-radius:100px;font-size:13px;z-index:9999;font-weight:700;animation:tin .25s ease;pointer-events:none;white-space:nowrap;box-shadow:0 4px 20px rgba(0,0,0,.2)}
        @keyframes tin{from{opacity:0;transform:translateX(-50%) translateY(-8px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}
        .ex-card{background:#fff;border-radius:14px;margin:0 16px 10px;box-shadow:0 1px 3px rgba(0,0,0,.06);overflow:hidden}
        .food-row{padding:12px 16px;border-bottom:1px solid #F9F9F9;cursor:pointer;display:flex;justify-content:space-between;align-items:center;-webkit-tap-highlight-color:transparent;transition:background .1s}
        .food-row:active{background:#F5F5F5}
        .stat-ring{transform:rotate(-90deg);transform-origin:50% 50%}
        .day-chip{padding:8px 16px;border-radius:100px;font-size:12px;font-weight:700;cursor:pointer;border:2px solid #E0E0E0;background:#fff;color:#999;font-family:inherit;transition:all .2s;-webkit-tap-highlight-color:transparent}
        .sleep-quality{display:flex;gap:6px;margin-top:8px}
        .sq-block{flex:1;height:6px;border-radius:3px;transition:background .3s}
      `}</style>
